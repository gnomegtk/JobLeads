    ...
    {%- else -%}
        <div {{ block('widget_container_attributes') }}>
            ...
            <table class=\"{{ table_class|default('') }}\" role=\"presentation\">
                <thead class=\"thead-dark\">
                    <tr>
                        {%- if with_years %}<th scope=\"col\">{{ form_label(form.years) }}</th>{% endif -%}
                        {%- if with_months %}<th scope=\"col\">{{ form_label(form.months) }}</th>{% endif -%}
                        {%- if with_weeks %}<th scope=\"col\">{{ form_label(form.weeks) }}</th>{% endif -%}
                        {%- if with_days %}<th scope=\"col\">{{ form_label(form.days) }}</th>{% endif -%}
                        {%- if with_hours %}<th scope=\"col\">{{ form_label(form.hours) }}</th>{% endif -%}
                        {%- if with_minutes %}<th scope=\"col\">{{ form_label(form.minutes) }}</th>{% endif -%}
                        {%- if with_seconds %}<th scope=\"col\">{{ form_label(form.seconds) }}</th>{% endif -%}
                    </tr>
                </thead>
                <tbody>
                    <tr>
...
{% block attributes -%}
    ...
    {%- endfor -%}
{%- endblock attributes -%}
", "form_div_layout.html.twig", "/data/jobleads/project/vendor/symfony/twig-bridge/Resources/views/Form/form_div_layout.html.twig");
    }
}
